...
const ProductModal = ({ setShowModal, addProduct }) => {
  ...
  const handleBrandChange = (e) => {
    ...
    if (brandId) {
      ...
      axiosInstance
        .get(`/user/products/prices?brandId=${brandId}`)
  ...
  const handlePriceChange = (e) => {
    ...
    if (priceId) {
      ...
      axiosInstance
        .get(`/user/products/variants?priceId=${priceId}`)
  ...
  const handleAddProduct = () => {
    const selectedProduct = variants.find(
      (variant) => variant.id === Number(selectedVariant)
    );
    console.log(selectedProduct.stock);
    if (quantity > selectedProduct.stock) {
        window.alert("Not enough Stock of " + selectedProduct.description +", please check again.")
      console.log("Not enough Stock, please check again.");
    } else {
      const product = {
        variantId: Number(selectedVariant),
        brandName: brands.find((brand) => brand.id === Number(selectedBrand))
          .name,
        price: prices.find((price) => price.id === Number(selectedPrice)).price,
        description: selectedProduct.description,
        hsnCode: selectedProduct.hsnCode,
        taxRate: selectedProduct.taxRate,
        cessRate: selectedProduct.cessRate,
